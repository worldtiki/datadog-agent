...
int __attribute__((always_inline)) is_discarded_by_inode(u64 event_type, u32 mount_id, u64 inode) {
    struct inode_discarder_t key = {
        .path_key = {
            .ino = inode,
            .mount_id = mount_id,
...
int __attribute__((always_inline)) is_discarded_by_process(const char mode, u64 event_type) {
    if (mode != NO_FILTER) {
        ...
        if (entry && is_discarded_by_inode(event_type, entry->executable.mount_id, entry->executable.inode)) {
            return 1;
        }
    }
